...
import { Loading01 } from "@hm/icons";
...
export const IconLeftNumber = <T extends ComponentType>({
    ...
    const statuses = {
        ...
        current: {
            // Ring sin color sólido — el ring giratorio (absolute below)
            // es el indicador visual de "en proceso" pedido por el usuario.
    ...
    return (
        <div className={cx("group flex h-max flex-row items-start justify-start gap-4", status === "incomplete" && "opacity-60", size === "sm" && "gap-3")}>
            <div className="flex flex-col items-center self-stretch">
                <span className={cx("relative z-10 flex shrink-0 items-center justify-center rounded-full", circleSize)}>
                    {/* Background del círculo — gris para current/incomplete,
                        success para complete. Vive bajo el ring giratorio. */}
                    <span
                        aria-hidden="true"
                        className={cx(
                            ...
                            statuses[status].icon,
                            status === "incomplete" && "opacity-60",
                            // Para `current`, ocultamos el ring estático: lo
                            // sustituye el ring giratorio del overlay siguiente.
                            status !== "current" && status !== "complete" && "ring-1 ring-inset ring-secondary",
                        )}
                    />
                    {status === "current" && (
                        // T-07 · Spinner UU canónico (`Loading01` rotando)
                        // en vez del SVG hand-rolled border-2 / border-t-X.
                        // Cumple regla 0.4: "Spinners salen de Loading01
                        // de @hm/icons". Misma sustitución que
                        // wizard-sidebar y la decisión de la regla 0.4 §0.
                        <Loading01
                            aria-hidden="true"
                            className="absolute inset-0 size-full animate-spin text-brand-solid motion-reduce:animate-none"
                        />
                    )}
                    <span className="relative z-10">
                        {status === "complete" ? (
                            ...
                            <span className={cx("font-semibold", statuses[status].title, size === "sm" ? "text-xs" : "text-sm")}>{step}</span>
                        )}
                    </span>
                </span>
...
export const SpotlightIconTop = <T extends ComponentType>({
    status = "incomplete",
    connector = true,
    size = "sm",
    icon: Icon,
...
export const SpotlightIconLeft = <T extends ComponentType>({
    status = "incomplete",
    connector = true,
    size = "sm",
    icon: Icon,
    ...
    return (
        <div className={cx("group flex flex-row gap-4", status !== "current" && "opacity-60", size === "sm" && "gap-3")}>
            <div className="flex flex-col items-center self-stretch">
                <SpotlightIcon size={size === "sm" ? "md" : "lg"} color="gray" theme="modern">
                    {Icon && <Icon />}
                </SpotlightIcon>
