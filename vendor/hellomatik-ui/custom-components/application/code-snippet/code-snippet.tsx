...
import { Fragment, createContext, type HTMLAttributes, type ReactNode, type RefObject, use, useEffect, useLayoutEffect, useRef, useState } from "react";
import { Check, Copy01, Expand06 } from "@hm/icons";
    ...
    const out = await codeToHast(code, {
        ...
        colorReplacements: {
            "github-light": {
                ...
                // Post-audit F2: `--color-utility-brand-600` (Hellomatik green
                // #099250) sobre white = 4.0:1 — falla WCAG AA (necesita ≥4.5:1
                // para texto normal). Cambio a `utility-purple-700` que mantiene
                // la semántica de "keyword" original de shiki (#6f42c1 era ya
                // purple) y ofrece contraste ≥7:1.
                "#6f42c1": "var(--color-utility-purple-700)",
            ...
            "github-dark": {
                ...
                "#b392f0": "var(--color-utility-purple-700)",
...
const CodeSnippetComponent = ({
    ...
    const isInsideTabs = use(TabsRefContext) !== null;
        ...
        >
            ...
            {(codeText || onExpand) && (
                <div className="absolute top-2 right-2 z-10 flex items-center gap-1 opacity-0 transition duration-100 ease-linear group-hover/code-snippet:opacity-100 has-[:focus-visible]:opacity-100">
                    {onExpand && <UtilityButton size="xs" color="tertiary" icon={Expand06} tooltip="Expand" onClick={onExpand} />}
                    {codeText && <UtilityButton size="xs" color="tertiary" icon={copied ? Check : Copy01} tooltip="Copy" onClick={() => copy(codeText)} />}
                </div>
            )}
        </div>
    );
...
const CodeSnippetTabList = <T extends object>({ children, className, ...otherProps }: TabListProps<T>) => {
    const { copy, copied } = useClipboard();
    const tabsRef = use(TabsRefContext);
    ...
    return (
        <div className="relative flex items-start">
            ...
            <div className="flex shrink-0 items-center pt-2 pr-2">
                <UtilityButton size="xs" color="tertiary" icon={copied ? Check : Copy01} tooltip="Copy" onClick={handleCopy} />
            </div>
        </div>
    );
};
