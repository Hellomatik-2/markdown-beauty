...
import { Children, Fragment, createContext, use, useMemo, useRef, useState } from "react";
...
export const CommandMenuList = ({ className, selectionMode = "single", ...props }: CommandMenuListProps) => {
    const { hasFooter, emptyState, ...listBoxProps } = use(CommandMenuContext);
    ...
    return (
        <AriaListBox
            ...
            // A11y: WCAG SC 2.1.1 `scrollable-region-focusable` exige que un
            // contenedor scrollable sea alcanzable por Tab (no solo arrows).
            // react-aria ListBox NO añade tabIndex al contenedor por defecto
            // (focus va a items via virtual focus + arrows). Pero un usuario
            // que entra al palette y presiona Tab no puede llegar al listbox
            // para scrollear manualmente.
            //
            // Fix: tabIndex=0 vía ref-callback (no hay prop tipado). Esto:
            //   - axe satisfecho (rule pass)
            //   - usuario puede Tab al listbox para scroll keyboard
            //   - arrow keys siguen moviendo entre options (react-aria override)
            //   - No rompe el virtual focus pattern
            ref={(el) => { if (el && el.getAttribute("tabindex") !== "0") el.setAttribute("tabindex", "0"); }}
            className={(state) =>
                cx(
...
export const CommandMenuPreview = ({ children, asChild, ...props }: CommandMenuPreviewProps) => {
    const { selectedKeys } = use(CommandMenuContext);
