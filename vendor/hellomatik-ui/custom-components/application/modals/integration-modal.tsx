...
import { BookOpen01, Check, Copy01, Link01, SwitchHorizontal01 } from "@hm/icons";
...
const AppIcon = ({ src, alt }: { src: string; alt: string }) => (
    <div className={cx("relative size-12 rounded-xl bg-primary_alt p-1 ring-1 ring-primary ring-inset sm:size-14 sm:rounded-[14px]", "")}>
        <Image
            ...
            unoptimized
            className={cx(
                "rounded-lg object-cover sm:rounded-[10px]",
...
export const IntegrationModal = () => {
    ...
    const linkUrl = "assets.local/integrations/linear";

    return (
        <AppModalOverlay isOpen={isOpen} onOpenChange={setIsOpen} isDismissable>
            <AppModal>
                <AppDialog>
                    <div className="relative w-full overflow-hidden rounded-2xl bg-primary shadow-xl sm:max-w-110 mx-auto">
                        {/* Integration logos */}
                        <div className="flex items-center justify-center gap-3 px-4 pt-6 sm:pt-8">
                            <AppIcon src="https://assets.local/logos/images/Shutterframe.jpg" alt="Acme" />
                            <SwitchHorizontal01 className="size-5 text-fg-quaternary" />
                            <AppIcon src="https://assets.local/logos/images/Linear.png" alt="Linear" />
                        </div>

                        {/* Header */}
                        <div className="flex flex-col items-center gap-0.5 px-4 pt-4 text-center sm:px-6">
                            <AriaHeading slot="title" className="text-md font-semibold text-primary">
                                Connect Acme to Linear
                            </AriaHeading>
                        ...
                        <div className="flex flex-col gap-4 px-4 sm:gap-5 sm:px-6 sm:pb-8">
                            <div className="flex flex-col gap-3">
                                <p className="text-sm font-semibold text-primary">Acme would like to</p>
