...
import { Dropper, Plus } from "@hm/icons";
...
const ColorFormatSelect = ({ className, ...props }: Omit<SelectProps, "children">) => {
    ...
    return (
        <Select
            size="sm"
            aria-label={t("colorFormatAriaLabel")}
...
export const SwatchItem = ({ color, size = "md", ...props }: SwatchItemsProps) => (
    ...
    >
        <AriaColorSwatch className="size-full rounded-full ring-1 ring-alpha-black/10 ring-inset dark:ring-alpha-black/30" style={({ defaultStyle }) => defaultStyle} />
    </AriaColorSwatchPickerItem>
);
    ...
    return (
        <div className="flex flex-1 shadow-xs">
            <InputCell position="first" className="flex-1">
                <AriaColorField
                    aria-label={t("hexColorAriaLabel")}
                    value={state.color}
                    onChange={(c) => {
        ...
        <div className="flex flex-1 shadow-xs">
            {config.channels.map((ch, i) => (
                <InputCell key={ch} position={i === 0 ? "first" : "middle"} className="flex-1">
                    <AriaColorField
                        aria-label={t("channelAriaLabel", { channel: ch.charAt(0).toUpperCase() + ch.slice(1) })}
                        channel={ch}
                ...
                <div className="flex flex-wrap gap-2">
                    {colors.map((c, i) => (
                        <button
                            ...
                            style={{ background: c }}
                            aria-label={t("selectGradientAriaLabel")}
                        />
                    ))}
                </div>
            )}
