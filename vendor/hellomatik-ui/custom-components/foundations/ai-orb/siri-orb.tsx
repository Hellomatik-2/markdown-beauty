...
const SiriOrb: React.FC<SiriOrbProps> = ({
  ...
  return (
    <div
      className={cx("siri-orb", className)}
      style={
        {
          width: size,
          height: size,
    ...
    >
      {/* `@property --angle` se registra globalmente en styles/globals.css —
          CSS @property debe ser top-level, no puede vivir dentro de un
          <style> nested en el árbol React (hydration warnings + scope
          frágil en algunos browsers). */}
      <style>{`
        .siri-orb {
          display: grid;
        ...
        .siri-orb::before {
          /* animation-delay (negative, in seconds) permite sincronizar
             fases entre múltiples instancias mounted en momentos distintos.
             El padre setea --animation-delay según un timeline-origin global. */
