...
import type { FileIcon } from "@hm/file-icons";
import { FileIcon as FileTypeIcon } from "@hm/file-icons";
import { CheckCircle, Trash02, UploadCloud02, XCircle } from "@hm/icons";
...
export const FileUploadDropZone = ({
    ...
    return (
        <div
            ...
            onClick={(e) => {
                if (isDisabled) return;
                // Evita doble-trigger cuando el click viene del label/botón internos
            ...
            className={cx(
                ...
                isDisabled ? "cursor-not-allowed bg-secondary" : "cursor-pointer hover:bg-primary_hover",
                className,
            )}
        >
            <SpotlightIcon icon={UploadCloud02} color="gray" theme="modern" size="md" className={cx(isDisabled && "opacity-50")} />
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                {isComplete && <p className="text-sm font-medium text-success-primary">{t("complete")}</p>}

                                {!isComplete && !failed && <UploadCloud02 className="stroke-[2.5px size-4 text-fg-quaternary" />}
                                {!isComplete && !failed && <p className="text-sm font-medium text-quaternary">{t("uploading")}</p>}

                                {failed && <XCircle className="size-4 text-fg-error-primary" />}
                                {failed && <p className="text-sm font-medium text-error-primary">{t("failed")}</p>}
                            </div>
                        </div>
                    </div>

                    <UtilityButton color="tertiary" tooltip="Delete" icon={Trash02} size="xs" className="-mt-2 -mr-2 self-start" onClick={onDelete} />
                </div>
...
export const FileListItemProgressFill = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <m.li layout="position" className={cx("relative flex gap-3 overflow-hidden rounded-xl bg-primary p-4", className)}>
            {/* Progress fill. */}
            <div
