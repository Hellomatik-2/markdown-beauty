...
import { cloneElement, createContext, isValidElement, use, useCallback, useEffect, useState } from "react";
...
const CarouselIndicatorGroup = ({ children, ...props }: CarouselIndicatorGroupProps) => {
    ...
    // Bug fix: `.map((index) => ...)` ANTES pasaba el ELEMENTO de scrollSnaps
    // (array de scroll positions en px) como `index` — no era el índice del array.
    // Cuando todos los snaps eran 0 (caso inicial), todos los indicators recibían
    // index=0 → keys duplicados `slide-0` × N → React warning.
    // Fix: `.map((_, idx) => ...)` — pasamos el índice REAL del array.
    if (typeof children === "function") {
        return <nav {...props}>{scrollSnaps.map((_, idx) => children({ index: idx }))}</nav>;
    }
