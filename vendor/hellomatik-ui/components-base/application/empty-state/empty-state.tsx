...
import { FileIcon } from "@hm/file-icons";
import { SearchLg } from "@hm/icons";
import { SpotlightIcon as SpotlightIconbase } from "@/components/foundations/spotlight-icon/spotlight-icon";
...
const SpotlightIcon = ({ color = "gray", theme = "modern", icon = SearchLg, size, ...props }: ComponentPropsWithRef<typeof SpotlightIconbase>) => {
    const { size: rootSize } = use(RootContext);

    return <SpotlightIconbase {...props} {...{ color, theme, icon }} size={!size && rootSize === "lg" ? "xl" : size || "lg"} />;
};
        ...
        >
            ...
            {avatars.slice(0, avatarSlots.length).map((avatar, i) => {
                ...
                return (
                    <div
                        key={i}
                        className={cx("absolute top-1/2 left-1/2 relative rounded-full bg-primary p-px shadow-xs ring-[0.5px] ring-black/10", slot.size)}
                        style={{ transform: `translate(calc(-50% + ${x}px), calc(-50% + ${y}px))` }}
                    >
                        <Image src={avatar.src} alt="" fill sizes="48px" unoptimized className="rounded-full object-cover outline-[0.5px] -outline-offset-[0.5px] outline-black/16" />
                    </div>
                );
            })}
        </div>
...
const AvatarRow = ({ avatars = [], children, ...props }: AvatarRowProps) => {
    ...
    const renderAvatar = (avatar: { src: string; alt?: string }, sizeClass: string, key: number) => (
        ...
        >
            <Image src={avatar.src} alt={avatar.alt || ""} fill sizes="48px" unoptimized className="object-cover" />
        </div>
    );
...
const EmptyState = Root as typeof Root & {
    ...
    SpotlightIcon: typeof SpotlightIcon;
...
EmptyState.SpotlightIcon = SpotlightIcon;
