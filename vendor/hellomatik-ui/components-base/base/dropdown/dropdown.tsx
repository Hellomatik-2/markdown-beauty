...
import { Check, ChevronRight, DotsVertical } from "@hm/icons";
import type {
...
interface DropdownItemProps extends AriaMenuItemProps {
    ...
    /** Visual tone — "danger" renders text and icon in error/destructive color. */
    tone?: "neutral" | "danger";
}

const DropdownItem = ({ label, children, addon, icon: Icon, avatarUrl, unstyled, selectionIndicator = "checkmark", tone, ...props }: DropdownItemProps) => {
        ...
        (state: MenuItemRenderProps & { className?: string }) => {
            if (selectionIndicator === "checkmark") {
                return (
                    <Check
                        aria-hidden="true"
                        className={cx("size-4 shrink-0 stroke-[2.25px] text-fg-secondary", !state.isSelected && "invisible", state.className)}
                    />
                );
            }
        ...
        >
            {(state) => (
                ...
                >
                    ...
                    {Icon && <Icon aria-hidden="true" className={cx("mr-2 size-4 shrink-0 stroke-[2.25px]", tone === "danger" ? "text-fg-error-primary" : "text-fg-quaternary")} />}

                    <span className={cx("grow truncate text-sm font-semibold", tone === "danger" ? "text-fg-error-primary" : "text-secondary", state.isFocused && (tone === "danger" ? "text-fg-error-primary" : "text-secondary_hover"))}>
...
const DropdownPopover = (props: DropdownPopoverProps) => {
    return (
        <AriaPopover
            ...
            className={(state) =>
                cx(
                    "w-62 origin-(--trigger-anchor-point) overflow-clip rounded-lg bg-primary shadow-lg ring-1 ring-secondary_alt will-change-transform",
                    state.isEntering &&
...
const DropdownSeparator = (props: AriaSeparatorProps) => {
    return <AriaSeparator {...props} className={cx("my-1 h-px w-full bg-border-tertiary", props.className)} />;
};
