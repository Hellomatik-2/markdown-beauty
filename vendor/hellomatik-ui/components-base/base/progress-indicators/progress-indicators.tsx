...
export interface ProgressBarProps {
    ...
    /**
     * Accessible label for the progressbar. Required by WCAG when there is
     * no visible label adjacent (axe rule `aria-progressbar-name`).
     */
    "aria-label"?: string;
    /**
     * ID of the element that labels this progressbar (preferred over
     * `aria-label` when a visible label exists in the DOM).
     */
 ...
 */
export const ProgressBarBase = ({ value, min = 0, max = 100, className, progressClassName, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy }: ProgressBarProps) => {
 ...
 */
export const ProgressBar = ({ value, min = 0, max = 100, valueFormatter, labelPosition, className, progressClassName, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy }: ProgressIndicatorWithTextProps) => {
    const percentage = ((value - min) * 100) / (max - min);
    const formattedValue = valueFormatter ? valueFormatter(value, percentage) : `${percentage.toFixed(0)}%`; // Default to rounded percentage

    const baseProgressBar = <ProgressBarBase min={min} max={max} value={value} className={className} progressClassName={progressClassName} aria-label={ariaLabel} aria-labelledby={ariaLabelledBy} />;
