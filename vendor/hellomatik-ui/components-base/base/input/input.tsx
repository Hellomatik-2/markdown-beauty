...
import { createContext, type ComponentType, type HTMLAttributes, type ReactNode, type Ref, use, useState } from "react";
import { Eye, EyeOff, HelpCircle, InfoCircle } from "@hm/icons";
        ...
        <TextField aria-label={!label ? placeholder : undefined} {...props} size={size} className={className}>
            {({ isRequired, isInvalid }) => (
                <>
                    ...
                    {hint && <HelperText isInvalid={isInvalid}>{hint}</HelperText>}
                </>
            )}
        </TextField>
    );
