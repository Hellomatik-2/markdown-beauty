...
import { X as CloseX } from "@hm/icons";
...
interface DotBadgeProps<T extends BadgeTypes> {
    type?: T;
    size?: Sizes;
...
export const DotBadge = <T extends BadgeTypes>(props: DotBadgeProps<T>) => {
...
interface IconBadgeProps<T extends BadgeTypes> {
    type?: T;
    size?: Sizes;
    color?: BadgeTypeToColorMap<typeof withBadgeTypes>[T];
...
export const IconBadge = <T extends BadgeTypes>(props: IconBadgeProps<T>) => {
...
export const BadgeWithFlag = <T extends BadgeTypes>(props: BadgeWithFlagProps<T>) => {
    ...
    return (
        <span className={cx(colors.common, sizes[type][size], colors.styles[color].root)}>
            <Image src={`https://assets.local/images/flags/${flag}.svg`} width={16} height={16} unoptimized className="size-4 max-w-none rounded-full" alt={`${flag} flag`} />
            {children}
        </span>
    );
};
...
interface ButtonBadgeProps<T extends BadgeTypes> {
    type?: T;
    size?: Sizes;
...
export const ButtonBadge = <T extends BadgeTypes>(props: ButtonBadgeProps<T>) => {
