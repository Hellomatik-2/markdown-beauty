...
export const styles = sortCx({
    ...
    colors: {
        ...
        "primary-solid": {
            // Neutral solid surface (no brand color). Auto-inverts in dark theme
            // via the `*-solid` design tokens — used in auth CTAs sitting on
            // the white left panel of AuthShell.
            root: [
                "bg-primary-solid text-primary_on-brand shadow-xs-skeuomorphic ring-1 ring-transparent ring-inset hover:bg-secondary-solid data-loading:bg-secondary-solid",
                // Inner border gradient (mirrors primary)
                "before:absolute before:inset-px before:border before:border-white/12 before:mask-b-from-0%",
                // Icon styles
                "*:data-icon:text-white/60 hover:*:data-icon:text-white/70",
            ].join(" "),
 ...
 */
export interface ButtonProps extends SharedProps, DetailedHTMLProps<Omit<ButtonHTMLAttributes<HTMLButtonElement>, "color" | "slot">, HTMLButtonElement> {
 ...
 */
interface LinkProps extends SharedProps, DetailedHTMLProps<Omit<AnchorHTMLAttributes<HTMLAnchorElement>, "color">, HTMLAnchorElement> {
