...
import { createContext, type ImgHTMLAttributes, type PropsWithChildren, type RefAttributes, use, useState } from "react";
import { User01 } from "@hm/icons";
import {
    Tag as AriaTag,
...
export const TagAvatar = ({ src, alt, contrastBorder = true, className }: ImgHTMLAttributes<HTMLImageElement> & { contrastBorder?: boolean }) => {
    ...
    return (
        ...
        >
            {src && !isFailed ? (
                <Image data-avatar-img fill sizes="48px" className="object-cover" src={src as string} alt={alt ?? ""} unoptimized onError={() => setIsFailed(true)} />
            ) : (
...
export const Tag = ({
    ...
    const context = use(TagGroupContext);
